...
class InfoB4training(discord.ui.View):
    ...
    @discord.ui.button(
        label="Continue to flight training",
    ...
    async def first_button_callback(self, button, interaction):
        ...
        if role in author.roles:
            ...
            await channel.send(
                f"{interaction.user.mention} continue here, run </va training:1016059999056826479> and input your desired destination and origin."
            )
...
class VACommands(discord.Cog):
    ...
    async def get_airports_d(self, ctx: discord.AutocompleteContext):
        return [
            ...
            if destination.startswith(ctx.value.upper())
        ]
    ...
    async def vasetup(self, ctx):
        ...
        ⌞______**2X**______⌟
        -An instructor will check you off once again for the final time, you can then fly as much as you want for the VA!""",
            color=cfc,
        )
        channel1 = self.bot.get_channel(1040927466975404054)
        channel2 = self.bot.get_channel(1041057335449227314)
        await channel1.send(embed=embed, view=InfoB4training(bot=self.bot))
        embed = discord.Embed(
            title="Required plugin: StableApproach",
            description="""
        ...
        class TypeView(discord.ui.View):
            ...
            @discord.ui.select(
                ...
                options=[
                    discord.SelectOption(
                        label="Boeing 737-200", description="By FlyJSim"
                    ),
                    discord.SelectOption(label="Boeing 737-800", description="By Zibo"),
                    discord.SelectOption(
                        label="Boeing 757-200", description="By Flight Factor"
                    ),
                    discord.SelectOption(
                        label="Airbus A300-600", description="By iniSimulations"
                    ),
                    discord.SelectOption(
                        label="Airbus A300-600F", description="By iniSimulations"
                    ),
                ],
            )
            async def select_callback(self, select, interaction):
                ...
                if select.values[0] == "Boeing 737-200":
                    embed = discord.Embed(
                        title="You have selected the Boeing 737-200 as your type rating.",
                        description="Run  </va training:1016059999056826479> again to file your first flight.",
                        color=cfc,
                    )
                ...
                if select.values[0] == "Boeing 737-800":
                    embed = discord.Embed(
                        title="You have selected the Boeing 737-800 as your type rating.",
                        description="Run  </va training:1016059999056826479> again to file your first flight.",
                        color=cfc,
                    )
                    await interaction.response.send_message(embed=embed)
                ...
                if select.values[0] == "Boeing 757-200":
                    embed = discord.Embed(
                        title="You have selected the Boeing 757-200 as your type rating.",
                        description="Run  </va training:1016059999056826479> again to file your first flight.",
                        color=cfc,
                    )
                    await interaction.response.send_message(embed=embed)
                ...
                if select.values[0] == "Airbus A300-600":
                    embed = discord.Embed(
                        title="You have selected the Airbus A300-600 as your type rating.",
                        description="Run  </va training:1016059999056826479> again to file your first flight.",
                        color=cfc,
                    )
                    await interaction.response.send_message(embed=embed)
                ...
                if select.values[0] == "Airbus A300-600F":
                    embed = discord.Embed(
                        title="You have selected the Airbus A300-600F as your type rating.",
                        description="Run  </va training:1016059999056826479> again to file your first flight.",
                        color=cfc,
                    )
                    await interaction.response.send_message(embed=embed)
        ...
        if os.path.exists(f"ClearFly_VA/users/{user.id}/student.ini"):
            ...
            if config.get("Student", "hasAccess") == "1":
                if config.get("Student", "typed") == "0":
                    embed = discord.Embed(
                        title="Choose the aircraf you want to fly!",
                        description="In order to fly for the VA you will need to get a type rating too, to do this select the aircraft you want below and you will be prompted to give origin and a destination. An instructor will approve your flight(just like in your general training), you will need to do 2 flights and then an instructor will check you off once again and then you're good to go to do as many flights for the VA as you want!",
                        color=cfc,
                    )
                    ...
                    if config.get("Student", "ready") == "1":
                        if os.path.exists(f"ClearFly_VA/users/{user.id}/type.txt"):
                            ...
                            if lines == 3:
                                await ctx.respond(
                                    "You have flown 2 times already, wait to get checked off!"
                                )
                                return
...
Have a nice and safe flight!
                        ...
                        await ctx.send(
                            f"<@&1040918528565444618> someone needs to get in the air for their flight, give them the required info!"
                        )
                    else:
                        await ctx.respond(
                ...
                if role in ctx.author.roles:
                    ...
                    if os.path.exists(f"ClearFly_VA/users/{user.id}/student.ini"):
                        ...
                        if lines == 5:
                            await ctx.respond(
                                "You have flown 4 times already, wait to get checked off!"
                            )
                            return
...
Have a nice and safe flight!
                    ...
                    embed = discord.Embed(
                        title="Error 403!",
                        description="You do not have the CF student role. \nGet it in {channel here} before using this command!",
                        color=errorc,
                    )
                    await ctx.respond(embed=embed)
...
Have a nice and safe flight!
                ...
                embed = discord.Embed(
                    title="Error 403!",
                    description="You do not have the <@&1040918463763468369> role. \nGet it in <#965686982304997466> before using this command!",
                    color=errorc,
                )
                await ctx.respond(embed=embed)
    ...
    @instructor.command(
        name="approve",
        description="✅ Approve a student's flight and give the required info to them.",
    )
    @option("comments", required=False)
    @commands.has_role(1040918528565444618)
    async def vaapprove(
        self,
        ...
        if config.get("Student", "typed") == "1":
            ...
            embed = discord.Embed(
                ...
                description=f"{user.mention}'s flight has been approved and they can take off now.",
                color=cfc,
            )
            embed.add_field(
                name="Approved with following data:",
                value=f"""
```
Route : {route.upper()}
...
```
    ...
    @instructor.command(
        name="check-off", description="☑️ Check off a user to end their training"
    )
    @commands.has_role(1040918528565444618)
    async def vacheckoff(self, ctx: discord.ApplicationContext, user: discord.Member):
        guild = self.bot.get_guild(965419296937365514)
        role = guild.get_role(1040918528565444618)
        config = configparser.ConfigParser()
        config.read(f"ClearFly_VA/users/{user.id}/student.ini")
        ...
        if config.get("Student", "typed") == "0":
            ...
            if lines == 5:
                if config.get("Student", "hasAccess") == "0":
                    ...
                    with open(
                        ...
                        config.write(configfile)
                    embed = discord.Embed(
                        title=f"{user} has been checked off.", color=cfc
                    )
                    await ctx.respond(embed=embed, ephemeral=True)
                    embed = discord.Embed(
                        title=f"{user} has finished training!",
                        description=f"Congratulations {user.mention}!\n Now you can do your type rating training and then fly as much as you want for the VA.",
                        colour=cfc,
                        timestamp=datetime.now(),
                    )
                    embed.add_field(
                        name="_ _", value=f"Checked off by {ctx.author.mention}"
                    )
                    await channel.send(f"{user.mention}", embed=embed)
                else:
                    embed = discord.Embed(
                        title="The user has been checked off already.", color=errorc
                    )
                    await ctx.respond(embed=embed, ephemeral=True)
            else:
                embed = discord.Embed(
                    title="The user hasn't completed enough flights.", color=errorc
                )
                await ctx.respond(embed=embed, ephemeral=True)
        else:
            ...
            if lines == 3:
                if config.get("Student", "end") == "0":
                    ...
                    await ctx.respond(embed=embed, ephemeral=True)
                    embed = discord.Embed(
                        title=f"{user} has fully finished training for the {actype}!",
                        description=f"Congratulations {user.mention}!\n Now you can fly as much as you want for the VA!",
                        colour=cfc,
                        timestamp=datetime.now(),
                    )
                    embed.add_field(
    ...
    @option(
        "aircraft",
        description="The aircraft you will use for the flight.",
        choices=["B732", "B738", "B752", "A306", "A306F"],
    )
    @option(
        "origin",
        description="The airport(ICAO) you will fly from.",
        autocomplete=get_airports_o,
    )
    @option(
        "destination",
        description="The airport(ICAO) you will fly to.",
        autocomplete=get_airports_d,
    )
    async def file(
        self,
        ctx: discord.ApplicationContext,
            ...
            if config.get("Student", "end") == "1":
                if (origin in airports) and (destination in airports):
                    ...
                    if not aircraft == config.get("Student", "type"):
                        embed = discord.Embed(
                            title="Error 503!",
                            description="You need to have a type rating of this aircraft if you want to fly it!",
                            color=errorc,
                        )
                        await ctx.respond(embed=embed)
                        return
                    ...
                    embed.add_field(
                        name="Your flight has been filed with the following data:",
                        value=f"""
    ```
    Aircraft:{aircraft}
    Departure:{origin}
    ...
    Have a nice and safe flight!
                    ...
                    if aircraft == "B732":
                        embed.set_thumbnail(
                            url="https://cdn.discordapp.com/attachments/1038060095902330952/1038065978019430430/FJS_732_TwinJet_icon11_thumb.png"
                        )
                    if aircraft == "B738":
                        embed.set_thumbnail(
                            url="https://cdn.discordapp.com/attachments/1038060053896364063/1038065018983432242/b738_4k_icon11_thumb.png"
                        )
                    if aircraft == "A300":
                        embed.set_thumbnail(
                            url="https://cdn.discordapp.com/attachments/1013239106198835300/1015290004001542164/A300_P_V2_-_2022-08-31_00.37.05.PNG"
                        )
                    if aircraft == "A300F":
                        embed.set_thumbnail(
                            url="https://cdn.discordapp.com/attachments/1038063084733997178/1038065483234164837/A300_F_V2_icon11_thumb.png"
                        )
                    await ctx.respond(embed=embed)
                else:
                    embed = discord.Embed(
                        title="Error 422!",
                        description="The origin/destination you provided are airports we do not fly to, please choose an airport that comes up while using the command.",
                        color=errorc,
                    )
                    await ctx.respond(embed=embed)
            else:
                embed = discord.Embed(
                    title="Error 503!",
                    description="You need to train before using this command",
                    color=errorc,
                )
                await ctx.respond(embed=embed)
        else:
    ...
    @va.command(
        ...
        description="🖊️ Something happened on your flight? Run this command and tell us what happened!",
    )
    @option(
        "flightnumber",
        description="The flight number of the flight where the accident happened.",
    )
    ...
    @va.command(
        name="divert",
        description="🛬 If you need to divert to another airport you can with this command.",
    )
        ...
        if os.path.exists(f"ClearFly_VA/users/{ctx.author.id}/student.ini"):
            ...
            if config.get("Student", "end") == "1":
                if not len(airport) == 4:
                    embed = discord.Embed(
                        title="Error 404!",
                        description="That doesn't seem to be a valid ICAO code",
                        color=errorc,
                    )
                    await ctx.respond(embed=embed)
                else:
    ...
    @va.command(
        name="cancel", description="❌ Cancels and removes your last filed flight."
    )
        ...
        if os.path.exists(f"ClearFly_VA/users/{ctx.author.id}/student.ini"):
            ...
            if config.get("Student", "end") == "1":
                if os.path.exists(f"ClearFly_VA/users/{ctx.author.id}/data.txt"):
                    ...
                    embed = discord.Embed(
                        title="Error 404!",
                        description="Didn't found any flights, you should file at least one flight to use this command!",
                        color=errorc,
                    )
                    await ctx.respond(embed=embed)
            else:
...
```
            ...
            if user == None:
                embed = discord.Embed(
                    title="Error 404!",
                    description=f"No flights were found for you, make sure you have flights filed!",
                    color=errorc,
                )
            else:
                embed = discord.Embed(
                    title="Error 404!",
                    description=f"No flights were found for {user.mention}, make sure they have flights filed!",
                    color=errorc,
                )
            await ctx.respond(embed=embed)
...
```
    ...
    @va.command(
        name="stats", description="📈 Show general statistics about the whole VA."
    )
    async def vastats(self, ctx: discord.ApplicationContext):
        cmnac = []
        output = 0
        cmndestoutput = []
        index = 0
        ...
        def delstr2(lst):
            return [f"{''.join(elem.split()[2:]).rstrip()}" for elem in lst]
        ...
        embed.add_field(
            name="_ _",
            value="\n*Notice: Both 'Most Common Aircraft' and 'Most Common Destination' will have a random selected value of 2 or more elements with the same frequency if that is the case.*",
            inline=True,
        )
        await ctx.respond(embed=embed)
    ...
    @va.command(
        name="leaderboard",
        description="📊 Get the leaderboard of who flew the most flights!",
    )
    async def valb(self, ctx: discord.ApplicationContext):
        await ctx.defer()
    ...
    async def valivs(self, ctx: discord.ApplicationContext, noauth: bool = False):
        if noauth == False:
            button1 = Button(
                label="Boeing 737-800 by Zibo",
                style=discord.ButtonStyle.url,
                url="https://drive.google.com/file/d/1bNXkHHlItE-MhfM6Nc-l5-W75zW9thYP/view?usp=share_link",
            )
            button2 = Button(
                label="Boeing 737-200 by FlyJSim",
                style=discord.ButtonStyle.url,
                url="https://drive.google.com/file/d/1o1vQk_HG1iJhJH1t_Z8cLBimDwxG90-C/view?usp=share_link",
            )
            button3 = Button(
                label="Airbus A300-600 by IniSimulations",
                style=discord.ButtonStyle.url,
                url="https://drive.google.com/file/d/1vdIOYlcM_2kNhooD_CTDE8UpoJ7mIvkk/view?usp=share_link",
            )
            button4 = Button(
                label="Airbus A300-600F by IniSimulations",
                style=discord.ButtonStyle.url,
                url="https://drive.google.com/file/d/1w6kd3H0VBiWoTlmcvvvyte5wbqHE7i5k/view?usp=share_link",
            )
            button5 = Button(
                label="Cessna 172SP by Laminar Research",
                style=discord.ButtonStyle.url,
